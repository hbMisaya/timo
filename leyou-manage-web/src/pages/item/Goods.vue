    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="recommendedGood(props.item)" v-if="props.item.recommended==0">推荐</v-btn>
          <v-btn icon @click="recommendedGood(props.item)" v-else>不推荐</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      ...
      async deleteGoods(oldGoods) {
        // 发起请求，查询商品详情和skus
        this.$http.get("/item/delete?id="+oldGoods.id)
          .then(resp => { // 这里使用箭头函数
          this.getDataFromServer();
        })
      },
      async recommendedGood(oldGoods) {
        // 发起请求，查询商品详情和skus
        this.$http.put("/item/recommendedGood?id="+oldGoods.id)
          .then(resp => { // 这里使用箭头函数
            this.getDataFromServer();
          })
      },
